...
export default async function CotizacionDetailPage({
  ...
  const result = await load(async () => {
    ...
    return getCotizacionById(supabase, id)
  })
  ...
  if (result.data.orden_trabajo_id) {
    redirect(`/repair-orders/${result.data.orden_trabajo_id}`)
  }
  ...
  return (
    <div>
      ...
      <CotizacionDetailClient cotizacion={result.data} />
    </div>
  )
}
