...
/** Entrega entregada pero pendiente de facturar (cliente mensual). */
...
/** Entregas 'por_facturar' (facturación mensual), con cliente. */
export async function listPorFacturar(supabase: DbClient): Promise<PorFacturar[]> {
  const { orgId } = await getAuthContext(supabase)
